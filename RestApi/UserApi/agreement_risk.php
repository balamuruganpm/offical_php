    ...
    if ($itemCount > 0) {
        ...
        if ($itemCount > 0) {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $terms_cons[] = $row['agreement_risk'];
            }
            echo json_encode(array("status" => true, "message" => "Agreement and Risks", "Data" => $terms_cons));
        } else {
            http_response_code(400);
            echo json_encode(array("status" => false, "message" => "No Agreement and Risks Found"));
        }
    } else {
